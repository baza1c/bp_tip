...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = РегламентированнаяОтчетностьКлиент.ИмяСобытияЗавершениеОтправкиВКонтролирующийОрган() 
		...
		Или (ИмяСобытия = "Неудачная отправка" Или ИмяСобытия = РегламентированнаяОтчетностьКлиент.ИмяСобытияУспешнаяОтправка())
		И ТипЗнч(Параметр) = Тип("Структура")
...
Функция ДанныеОтразитьВУчетеВсеУплаты()
	...
	Для Каждого ДанныеСтроки Из ТаблицаУплаты Цикл
		Если ДанныеСтроки.СуммаРасхождение > 0 Тогда

...
Процедура ЗапуститьФормированиеСпискаДокументовПрепятствующихОтключениюОпцииНаКлиенте(ИмяОпции, ФормаВладелец, ОповещениеЗакрытия = Неопределено)
	...
	ИначеЕсли ИмяОпции = "ИспользоватьПродажиЧерезМаркетплейс" Тогда
		ПараметрыФормы.Вставить("ИмяМетода", "Обработки.ФункциональностьПрограммы.СформироватьСписокОбъектовПрепятствующихОтключениюИспользоватьПродажиЧерезМаркетплейс");

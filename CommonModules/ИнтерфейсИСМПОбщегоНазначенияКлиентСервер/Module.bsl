...
Функция ПараметрыКодаМаркировкиПоДаннымРазрешительногоРежима(ЭлементДанных, ВидПродукции = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Значение <> Неопределено Тогда
		Если ПроверкаЧерезТСПИоТ Тогда
			ПараметрыКодаМаркировки.ГоденДо = СтроковыеФункцииКлиентСервер.СтрокаВДату(Значение);
		Иначе
	...
	Если Значение <> Неопределено Тогда
		Если ПроверкаЧерезТСПИоТ Тогда
			ПараметрыКодаМаркировки.ДатаПроизводства = СтроковыеФункцииКлиентСервер.СтрокаВДату(Значение);
		Иначе

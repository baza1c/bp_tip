...
Функция ОбработатьШтрихкодыПослеУточненияНаТСПИоТ(ДанныеКодовМаркировки, ПараметрыСканирования, ФормаУникальныйИдентификатор, АварийныйРежим) Экспорт
	...
	Если ОбщегоНазначенияИСМПКлиентСервер.ЭтоРасширеннаяВерсияГосИС()
		И ЗначениеЗаполнено(РезультатОбработки.АдресДереваУпаковок) Тогда
		ШтрихкодированиеИСМПВызовСервера.ДополнитьДеревоДаннымиРазрешительногоРежимаГИСМТ(РезультатОбработки.АдресДереваУпаковок, ДанныеКодовМаркировки);
	КонецЕсли;

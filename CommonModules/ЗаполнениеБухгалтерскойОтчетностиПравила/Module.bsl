...
Процедура ЗаполнитьНастройкиПоУмолчаниюОДДС(Процессор)
	...
	Если ПолучитьФункциональнуюОпцию("ОсуществляетсяРеализацияТоваровУслугКомитентов") Тогда
		ДобавитьПоказательСвернутогоДенежногоПотока(Процессор, "АгентПокупателя", НСтр("ru = 'Исключены суммы приобретений в интересах комитента'", КодЯзыка));
	КонецЕсли;
	...
	ДобавитьПоказательСвернутогоДенежногоПотока(Процессор, "НДС", НСтр("ru = 'НДС, представленный свернуто'", КодЯзыка));
...
//   * НоменклатурнаяГруппа - Массив Из СправочникСсылка.НоменклатурныеГруппы
...
Процедура СоздатьКлассифицируемыеОбъектыПродажи(МенеджерВременныхТаблиц, Показатели)
	...
	НоменклатурныеГруппы.Колонки.Добавить("НоменклатурнаяГруппа",  Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));
...
Процедура ПолучитьДанныеУчетаПродажи(Процессор)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ОборотыДетально.Субконто1 КАК Справочник.НоменклатурныеГруппы), ЗНАЧЕНИЕ(Справочник.НоменклатурныеГруппы.ПустаяСсылка)) КАК Объект,
...
Функция ВидыНалоговНДФЛ(КодыВидовДохода)
	
	ВидыНалогов = Новый Массив;
	Для Каждого Код Из КодыВидовДохода Цикл
		
		ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.НДФЛ, Ложь, , Код);
		ВидыНалогов.Добавить(ВидНалога);
...
Функция НалоговыеПлатежиНДФЛОплатыТруда()
	...
	Возврат ВидыНалоговНДФЛ(Справочники.ВидыНалоговИПлатежейВБюджет.КодыВидовДоходаНДФЛ_ОплатаТруда());
...
Функция НалоговыеПлатежиНДФЛДивидендов()
	...
	Возврат ВидыНалоговНДФЛ(Справочники.ВидыНалоговИПлатежейВБюджет.КодыВидовДоходаНДФЛ_Дивиденды());

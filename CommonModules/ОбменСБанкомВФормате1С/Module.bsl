...
Процедура ИсправитьОшибкиУчастникаСчет(СтрокаДанныхИзБанка, Участник) Экспорт
	...
	Для Каждого ВозможныйНомерСчета Из СловаУчастника Цикл
		...
		Если Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка" Или Проблема = "РасчетыЧерезБанкКорреспондент"
			...
			ВозможныйНомерСчетаБезСлеша = НРег(СокрЛП(СтрЗаменить(ВозможныйНомерСчета, "/", "")));
			НомерСчетаВВерхнемРегистре  = ВРег(ВозможныйНомерСчетаБезСлеша);
			...
			ИначеЕсли (Проблема = "РасчетыЧерезБанкНерезидент" Или Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка")
				...
				И (ПредыдущееСлово = ПризнакРС Или ПредыдущееСлово = ПризнакРСлат Или СтрНачинаетсяС(ПредыдущееСлово, "acc")) Тогда
				НомерСчета = ВозможныйНомерСчетаБезСлеша;
				Прервать;
			КонецЕсли;

...
Функция ДанныеСтатусаОтправкиСотрудникаПоУсловию(СистемаБронирования, ФизическоеЛицо)
	...
	ДанныеСотрудников = МестаРаботыСотрудников(ФизическоеЛицо, Неопределено, Неопределено, Ложь);
	Если ДанныеСотрудников.ТаблицаСотрудников.Количество() = 0 Тогда
	...
	Если ИспользоватьЗарплатаКадрыКорпоративнаяПодсистемы Тогда
		ПодразделенияСотрудника = ДанныеСотрудников.ТаблицаСотрудников.ВыгрузитьКолонку("МестоВСтруктуреПредприятия");
	КонецЕсли;
	...
	Иначе
		ДанныеСтатуса.ОтправляетсяПоУсловию = ?(ДанныеСотрудников.ТаблицаСотрудников.Количество() <> 0, Истина, Ложь);

...
//
// Параметры:
//   ОбъектДО - Строка - представление объекта Документооборота.
//   ОбъектДОID - Строка - идентификатор объекта Документооборота.
//   ОбъектДОТип - Строка - тип объекта Документооборота.
//   ФормаВладелец - ФормаКлиентскогоПриложения - форма, из которой вызвано открытие связей.
//
Процедура ПоказатьСвязиОбъекта(ОбъектДО, ОбъектДОID, ОбъектДОТип, ФормаВладелец = Неопределено) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОбъектДО", ОбъектДО);
	ПараметрыФормы.Вставить("ОбъектДОID", ОбъектДОID);
	ПараметрыФормы.Вставить("ОбъектДОТип", ОбъектДОТип);
	
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот3.Форма.СвязиОбъекта", ПараметрыФормы, ФормаВладелец);

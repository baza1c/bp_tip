	...
	Для Каждого СтрокаТаблицы Из РасчетыПоАренде Цикл
		Если СтрокаТаблицы.ТипСлагаемого = "Авансы" Тогда
			...
			Если Значение <> 0 Тогда
				...
				Отчет = ЗаполнениеБухгалтерскойОтчетности.НастройкиРасшифровкиПоказателейБалансаПоАренде(НастройкиОтчета);
...
#Область НачалоНоминал
	...
	Если КонтекстРаздела.ЕстьДисконтирование Тогда
		РасчетПоказателейБалансаПоАренднымОбязательствамГрафики.ЗаполнитьШаблонРасшифровкиПоАрендеПояснениеБалансаОбязательстваНачалоПериода(ШаблонАренда);
	КонецЕсли;
...
#Область КонецНоминал
	...
	ШаблонАренда = НСтр("ru = 'Отрицательное сальдо по [СчетуАренды] за вычетом относящегося к нему НДС (сальдо по кредиту [СчетаНДСАвансовВыданных]). Включаются суммы со сроком действия договора не ранее [СрокПогашенияКонец]
			|Расчет выполняется отдельно по каждому договору аренды (лизинга)[СуммарноПоФилиалам].'");
	Если КонтекстРаздела.ЕстьДисконтирование Тогда
		РасчетПоказателейБалансаПоАренднымОбязательствамГрафики.ЗаполнитьШаблонРасшифровкиПоАрендеПояснениеБалансаОбязательстваКонецПериода(ШаблонАренда);
	КонецЕсли;
...
Функция РасчетыПоАренде(КонтекстЗаполненияОтчета, КонтекстРаздела, НДСАвансов = Неопределено)
	...
	Контекст = РасчетПоказателейБалансаПоАренднымОбязательствам.НовыйКонтекстРасчета();
	...
	НачалоРасчетыПоАренде = РасчетПоказателейБалансаПоАренднымОбязательствам.ИсходныеДанныеПоАренднымОбязательствам(Контекст).АрендныеОбязательства;
	...
	КонецРасчетыПоАренде = РасчетПоказателейБалансаПоАренднымОбязательствам.ИсходныеДанныеПоАренднымОбязательствам(Контекст).АрендныеОбязательства;

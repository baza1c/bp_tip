...
// Проверяет поддерживает ли веб-сервис Документооборота функционал автоматического создания связей.
//
// Возвращаемое значение:
//   Булево
//
Функция ДоступенФункционалАвтоматическогоСозданияСвязей() Экспорт
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.19.1");
...
//   ТребуетсяСписокСоздаваемыхСвязей - Булево - требуется получение списка автоматически создаваемых связей.
//
// Возвращаемое значение:
//   Структура:
...
//       ** Ключ - ОпределяемыйТип.ИнтеграцияС1СДокументооборотВсеСсылкиПереопределяемый - объект ИС.
//       ** Значение - СписокXDTO - список создаваемых связей к передаче в ДО.
//
Функция ПредварительныеДанныеДляВыгрузкиВДО(Прокси, СписокОбъектовИС, СоответствиеПравилЗагрузкиВДООбъектамИС,
		ТребуетсяСписокВыражений = Истина, ТребуютсяПечатныеФормы = Истина,
		ТребуетсяСписокСоздаваемыхСвязей = Ложь) Экспорт
	...
	КонецЕсли;
	Если ТребуетсяСписокСоздаваемыхСвязей Тогда
	...
	КонецЕсли;
	Если ТребуетсяСписокСоздаваемыхСвязей И ПравилаДляСоздаваемыхСвязей.Количество() > 0 Тогда
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		СписокСоздаваемыхСвязей = Новый Массив;
		...
		Если ПравилоЗагрузкиВДО <> Неопределено Тогда
			...
			Если ТребуетсяСписокСоздаваемыхСвязей Тогда
				Для Каждого Элемент Из МассивВсехСоздаваемыхСвязей Цикл
					Если Элемент.object.id = ПравилоЗагрузкиВДО.id Тогда
						СписокСоздаваемыхСвязей = Элемент.addedRelations;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		КонецЕсли;
		Если ТребуетсяСписокСоздаваемыхСвязей Тогда
			Данные.СоответствиеСоздаваемыхСвязейОбъектамИС.Вставить(ОбъектИС, СписокСоздаваемыхСвязей);
		КонецЕсли;
	КонецЦикла;
...
Функция СоздатьОбъектыДОПоПравилам(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции) Экспорт
	...
	ДоступенФункционалАвтоматическогоСозданияСвязей = ДоступенФункционалАвтоматическогоСозданияСвязей();
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		Если ДоступенФункционалАвтоматическогоСозданияСвязей Тогда
			ЗаполнитьСписокСвязиОбъекта(
				Прокси,
				СтруктураСозданияОбъекта.relations,
				ОбъектИС,
...
//
Функция НастройкиСвязей(Прокси, СсылкаИз) Экспорт
	
	Запрос = НастройкиСвязейЗапрос(Прокси, СсылкаИз);
	Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(Прокси, Запрос);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, Ответ);
...
//
Функция НастройкиСвязейЗапрос(Прокси, СсылкаИз) Экспорт
...
//   НастройкиСозданияСвязей - СписокXDTO - список настроек создания связей, заданный в правилах
...
//
Функция ПодходящиеОбъектыДляДобавленияСвязей(ОбъектИС, Знач НастройкиСозданияСвязей, ИмяСобытия) Экспорт
	...
	Для Каждого НастройкаXDTO Из НастройкиСозданияСвязей Цикл
...
Процедура ДополнитьПодходящиеОбъектыДляСозданияСвязей(ЗначениеРеквизитаИС, ПодходящиеОбъекты, НастройкаСозданияСвязей,
	...
	Если Не ЗначениеЗаполнено(ЗначениеРеквизитаИС)
			Или ПодходящиеОбъекты.Найти(ЗначениеРеквизитаИС, "Ссылка") <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
// Параметры:
//   Прокси - WSПрокси - объект для подключения к web-сервисам Документооборота.
//   СвязиОбъекта - СписокXDTO - список связей, которые требуется создать.
//   ОбъектИС - ЛюбаяСсылка - ссылка на исходный объект ИС.
//   НастройкиСозданияСвязей - СписокXDTO - список настроек создания связей, заданный в правилах
//     загрузки данных в ДО.
//
Процедура ЗаполнитьСписокСвязиОбъекта(Прокси, СвязиОбъекта, ОбъектИС, НастройкиСозданияСвязей)
	
	Если НастройкиСозданияСвязей = Неопределено Или НастройкиСозданияСвязей.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Для Каждого ПодходящийОбъект Из ПодходящиеОбъектыДляДобавленияСвязей Цикл
			...
			Связь.object.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(Прокси);
			Связь.object.name = Строка(ОбъектИС);
			...
			Если ПодходящийОбъект.СвязанныйОбъектДО = Неопределено Тогда
				...
				Если ТипЗнч(НовыйОбъектДОИлиСообщение) = Тип("ОбъектXDTO") Тогда
					Связь.relatedObject = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
						Прокси, "DMRelatedObject");
					Связь.relatedObject.objectID = НовыйОбъектДОИлиСообщение.objectID;
					Связь.relatedObject.name = НовыйОбъектДОИлиСообщение.name;
				Иначе
					ТекстОшибки = СтрШаблон(
			...
			Иначе
				...
				Связь.relatedObject.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
					Прокси,
					ПодходящийОбъект.СвязанныйОбъектДО.ID,
					ПодходящийОбъект.СвязанныйОбъектДО.type);
				Связь.relatedObject.name = ПодходящийОбъект.СвязанныйОбъектДО.name;
			КонецЕсли;
			...
			Связь.relationType.objectID = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьObjectID(
				Прокси,
				ПодходящийОбъект.ТипСвязи.ID,
				ПодходящийОбъект.ТипСвязи.Тип);
			Связь.relationType.name = ПодходящийОбъект.ТипСвязи.Наименование;
			
			Связь.comment = ПодходящийОбъект.Комментарий;
			
			СвязиОбъекта.Добавить(Связь);

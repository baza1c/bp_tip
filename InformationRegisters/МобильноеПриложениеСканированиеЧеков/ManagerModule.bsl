...
Функция ТикетАутентификации() Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеИТС.КодОшибки) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Мобильное приложение Сканер чеков'"),
...
Процедура ОбработатьСообщениеШиныМобильныхПриложений(Отправитель, Сообщение) Экспорт
	...
	Если Отправитель = Неопределено И Сообщение.Свойство("Заголовки") Тогда
		// Шина не вернула содержимое сообщения. Дополнительные инструкции находятся в заголовках.
		ОбработатьЗаголовкиСообщения(Сообщение.Заголовки);
		Возврат;
	КонецЕсли;
...
Функция НачатьПроверкуСообщений() Экспорт
	...
	ИнтервалПроверки = УчетКассовыхЧековПодотчетныхЛицКлиентСервер.ИнтервалПроверкиЧековПодотчетныхЛиц();
	ДатаПоследнегоОбновления = УчетКассовыхЧековПодотчетныхЛиц.ДатаПоследнегоОбновленияКассовыхЧеков();
	ПроверятьНовыеСообщения = ТекущаяУниверсальнаяДата() >= ДатаПоследнегоОбновления + ИнтервалПроверки;
	
	Если Не ПроверятьНовыеСообщения Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УчетКассовыхЧековПодотчетныхЛиц.СохранитьДатуПоследнегоОбновленияКассовыхЧеков();
...
Процедура ОбработатьЗаголовкиСообщения(Заголовки)
	
	ТаймаутЗапросов = Заголовки.Получить("Retry-After");
	Если ТаймаутЗапросов <> Неопределено Тогда
		// В заголовке дата передается в формате ISO: YYYY-MM-DDThh:mm:ss.sssZ (часовой пояс UTC+0)
		// Дата обновления кассовых чеков также хранится в часовом поясе UTC+0. Таким образом, дата передается в нужном часовом поясе.
		// Метод десериализации даты строкой выполняет преобразование даты к локальному часовому поясу, если строка содержит сведения
		// о часовом поясе. Обрежем последний символ "Z", чтобы метод десериализации не выполнял преобразование часовых поясов.
		ТаймаутЗапросов = СтрЗаменить(ТаймаутЗапросов, "Z", "");
		ДатаСледующегоЗапроса = ОбщегоНазначенияБП.ДесериализоватьДатуISO(ТаймаутЗапросов);
		Если ЗначениеЗаполнено(ДатаСледующегоЗапроса) Тогда
			УчетКассовыхЧековПодотчетныхЛиц.СохранитьДатуПоследнегоОбновленияКассовыхЧеков(ДатаСледующегоЗапроса);
		КонецЕсли;
	КонецЕсли;

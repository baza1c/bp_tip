...
Функция СформироватьВыходнойФайл2022(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникам)
	...
	ИспользоватьФормат2025 = ВыборкаПоШапкеДокумента.Дата >= ПерсонифицированныйУчет.ДатаПостановленияАДВ2025();
	Если ИспользоватьФормат2025 Тогда 
	...
	Если ВыборкаПоРаботникам.Следующий() Тогда 
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникам.ДатаАктовойЗаписи) Тогда 
			...
			Если ИспользоватьФормат2025 Тогда 
				ФорматАДВ_2.АктоваяЗапись.Значение.УТ3__КодЗАГС.Значение = ВыборкаПоРаботникам.КодЗАГС;
			КонецЕсли;
		КонецЕсли;
		...
		Если ИспользоватьФормат2025 Тогда 
			ПерсонифицированныйУчет.УстановитьВыводВФайл(ФорматАДВ_2.АктоваяЗапись.Значение.УТ3__КодЗАГС, "Значение");
		КонецЕсли;
